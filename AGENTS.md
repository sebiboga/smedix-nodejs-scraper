...
## Project
SMEDIX scraper for peviitor.ro (Node.js, ESM, Jest)
...
- **Only the API parsing logic in `index.js`** (`fetchJobsPage`, `parseApiJobs`) is SMEDIX-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
# E2E tests (real Perficient API, SOLR conditional)
npm run test:e2e
