...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 36734466, name: 'SMEDIX LLC ST. LOUIS SUCURSALA CLUJ NAPOCA', statusLabel: 'Funcțiune' }
      ]));
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('SMEDIX')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('SMEDIX SRL');
      expect(capturedUrl).toContain(encodeURIComponent('SMEDIX SRL'));
    });
  });
  ...
  describe('getCompanyFromANAF', () => {
    ...
    it('should throw after exhausting retries', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('36734466')).rejects.toThrow();
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('36734466');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('36734466', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('36734466')).rejects.toThrow();
    });
  });
});
